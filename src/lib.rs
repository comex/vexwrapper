...
impl Ty {
    fn max_int_val(self) -> Option<u64> {
        match self {
            I1 =>  Some(1),
            I8 =>  Some(std::u8::MAX),
            I16 => Some(std::u16::MAX),
            I32 => Some(std::u32::MAX),
...
fn guess_type<Ref: Deref<Target=&TyExpr>>(ex: Expr<Ref>) -> Ty {
    match &ex {
        &Bin(op, ref left, ref right) => {
            let lty = left.ty;
            lef rty = right.ty;
            assert_eq!(lty, rty);
            match op {
        ...
        &ITE(ref iff, ref then, ref els) => {
            assert_eq!(iff.ty, Ty::I1);
...
trait Builder {
    type Ref: Clone + Deref<Target=&TyExpr>;
    fn build(&self, Expr<Ref>) -> Ref;
    fn bin(&self, op: Binop, left: Ref, right: Ref) -> Ref {
        self.build(Expr::Bin(op, left, right))
    }
    fn un(&self, op: Unop, arg: Ref) -> Ref {
        self.build(Expr::Un(op, arg))
    }
    fn ite(&self, iff: Ref, then: Ref, els: Ref) -> Ref {
        self.build(Expr::ITE(iff, then, els))
    }
    fn const_int(&self, ty: Ty, num: u64) -> Ref {
...
fn make_unex<B: Builder>(b: B, op: u32, x: B::Ref) -> B::Ref {
    match op {
        ...
        Iop_CmpNEZ8 | Iop_CmpNEZ16 | Iop_CmpNEZ32 | Iop_CmpNEZ64 =>
            b.bin(Binop::CmpNe, x, b.const_int(x.ty, 0)),
        =>
...
fn make_binex<B: Builder>(b: B, op: u32, l: B::Ref, r: B::Ref) -> B::Ref {
    ...
    match op {
        Iop_CmpORD32U | Iop_CmpORD64U => cmp_ord(Unsigned),
        Iop_CmpORD32S | Iop_CmpORD64S => cmp_ord(Signed),

        _ => b.bin(translate_unop(op), l, r),
    }
}
